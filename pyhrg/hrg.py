...
# ═══════════════════════════════════════════════════════════════════════
# 2. GRAPH CONSTRUCTION — one Numba pass over the pixels
# ═══════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════
# 3. THE GROW LOOP — heap-ordered, variance-constrained
# ═══════════════════════════════════════════════════════════════════════
...
def _hierarchical_grow_single(seed_id, row_ptr, col_idx, weights,
    ...
    for iteration in range(max_iters):
        ...
        # Anneal the threshold
        if anneal_lambda < 1.0 and iteration > 0:
            v_thresh *= anneal_lambda

        # Pop the most similar unvisited candidate
        cand = -1
        while heap:
...
class HierarchicalRegionGrower:
    ...
    Design notes, in case the shape of the code is surprising:

    - **Regions are trees.** Marker-based watershed on the inverted CHM,
      seeded with the tree tops, yields exactly one region per top. So the
      graph nodes are the detected trees, and growing merges trees rather
      than pixels. That is what corrects over-detected tree tops.
    - **Statistics merge in O(1).** A crown absorbs a whole region at a
      time, so counts/means/variances combine with the pairwise formula of
      Chan, Golub & LeVeque (1983) — no rescanning the pixels. The single
      pixel pass that builds the graph and the per-region statistics is the
      only place that touches every pixel, and it is Numba-compiled.
    - **Candidates come off a heap.** Neighbours are tried in order of edge
      weight, w = alpha*|dmu| + beta*|dsigma| + gamma/(border+1), so the
      most similar neighbour is considered first. The grow loop itself is
      plain Python: it is driven by a heap and set membership, and runs once
      per seed rather than once per pixel, so it is not the bottleneck.
    - **Claims are arbitrated, not overwritten.** Seeds grow independently,
      so two crowns can claim the same region. ``conflict_rule`` on
      :meth:`run_all` settles it on the data, which keeps the output
